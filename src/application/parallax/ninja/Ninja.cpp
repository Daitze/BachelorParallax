...
Ninja::Ninja(const Util::Math::Vector2D &position) : Util::Game::D2::Entity(TAG, position, Util::Game::D2::RectangleCollider(position, Util::Math::Vector2D(0.2, 0.2), Util::Game::Collider::STATIC)) {
    addComponent(new Util::Game::D2::LinearMovementComponent(*this));
}

void Ninja::initialize() {
    ninja = Util::Game::D2::Sprite("/initrd/ninja/run1.bmp",0.2, 0.2267,Util::Math::Vector2D(0, 0,0));
}
